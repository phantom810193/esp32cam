...
5. API 重點：
    ...
    - `GET /members`：瀏覽預寫會員的個人資料與 2025 年消費紀錄，也可從最新上傳儀表板的「會員資料」按鈕快速進入。
    - `GET /health`：基本健康檢查。
...
6. SQLite 會自動建立資料庫與 Demo 資料。辨識到新臉孔時，系統會先以 Rekognition 集合比對，若找不到則建立匿名
   ...
   內建示範資料目前涵蓋五種消費輪廓：甜點收藏家、幼兒園家長、健身族、家庭採買者與健康養生客層。前 3 位示範
   會員已在商場完成註冊，`member_profiles` 表中帶有會員狀態、入會日、點數餘額、基本聯絡方式與商場會員代號
   （ME0001~ME0003）；後 2 位仍屬匿名訪客，商場會員代號維持空白，但同樣儲存狀態、入會日期與必要聯絡資訊，
   方便後續補件。當資料表中存在尚未綁定 `member_id` 的預留列時，新的匿名 `MEMxxxxxxxxxx` 會優先填入空缺，之後
   才會依照一般流程自動產生新的系統編號。
