...
def _prepare_member_ad_context(member_id: str) -> tuple[dict[str, object], MemberProfile | None]:
    ...
    predicted_dict = None
    prediction_candidates: list[Any] = []
    creative_purchases: list[Purchase] = purchases
    if not is_seeded_guest:
        try:
            prediction_result = predict_next_purchases(
                purchases,
                profile=profile,
                insights=insights,
            )
            prediction_candidates = list(getattr(prediction_result, "items", []) or [])
            predicted_item = prediction_candidates[0] if prediction_candidates else None
            predicted_dict = _prediction_to_dict(predicted_item)
            history_window = list(getattr(prediction_result, "history", []) or [])
            if history_window:
                creative_purchases = history_window
        except Exception as exc:
            logging.warning("Prediction pipeline unavailable for %s: %s", member_id, exc)
        ...
        try:
            creative = gemini.generate_ad_copy(
                ...
                [
                    ...
                    for purchase in creative_purchases
                ],
                insights=insights,
                predicted=predicted_dict or {},
                audience=audience,
    ...
    context = build_ad_context(
        ...
        creative=creative,
        predicted_item=predicted_dict,
        prediction_items=prediction_candidates,
        audience=audience,
    )
...
def _synthetic_guest_creative(predicted: Mapping[str, Any]) -> AdCreative:
    ...
    detail_fragments = [fragment for fragment in (price_text,) if fragment]
